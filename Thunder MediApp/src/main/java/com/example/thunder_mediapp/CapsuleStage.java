...
public class CapsuleStage implements Initializable {
    @FXML
    private TableView<Medicine> capsuleTable;
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        select = capsuleTable.getSelectionModel().getSelectedItems() ;
        try {
            list = Database_Management.fletchDataFromMedicine("capsules") ;
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        capsuleTable.setItems(list);

        System.out.println("Capsule:" +l_name);
    }
    @FXML
